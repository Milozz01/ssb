...
from PyQt5.QtWidgets import QLabel, QLayout, QMessageBox
...
class Dialogs:
    @classmethod
    def errorfunc(cls):
        errormsgnst = QMessageBox()
        errormsgnst.setIcon(QMessageBox.Warning)
        errormsgnst.setText("Nullposition detected!")
        errormsgnst.setInformativeText('Please release...')
        errormsgnst.setWindowTitle("Contact Warning")
        errormsgnst.exec_()

    @classmethod
    def releasefunc(cls):
        releasemsg = QMessageBox()
        releasemsg.setIcon(QMessageBox.Information)
        releasemsg.setText("Successfully released Nullposition!")
        releasemsg.setInformativeText('')
        releasemsg.setWindowTitle("Nullposition released")
        releasemsg.exec_()

    @classmethod
    def infomsgfunc(cls):
        infomsgnst = QMessageBox()
        infomsgnst.setIcon(QMessageBox.Information)
        infomsgnst.setText("Successfully reset to Nullposition!")
        infomsgnst.setInformativeText('Please release...')
        infomsgnst.setWindowTitle("Success")
        infomsgnst.exec_()
...
class Ui_mainWindow(object):
    ...
    def setupUi(self, mainWindow):
        ...
        self.pushButton_3.clicked.connect(lambda: self.nullpoint())
        self.pushButton_4.clicked.connect(lambda: self.release())
    ...
    def retranslateui(self, mainWindow):
        ...
        self.pushButton_3.setText(_translate("mainWindow", "Nullposition"))
    ...
    @staticmethod
    def stop():
        print("App was closed, pressed STOP")
        sys.exit()

    def release(self):
        print("release Nullposition activated")
    ...
    def nullpoint(self):
        ...
        # pwm.start(dc_25)
        print("pwm = ", dc_25)
        print("Linearantrieb wird auf Nullposition gesetzt")
    ...
    def runmultimotorsetupv2(self, index, countermulti=0):
        ...
        if GPIO.input(gpio_input) == GPIO.LOW:
            ...
            GPIO.cleanup()
            Dialogs.errorfunc()
            # sys.exit("Nullposition detected")
            return
        else:
            while True and countermulti < 4:
                ...
                if GPIO.input(gpio_input) == GPIO.LOW:
                    ...
                    thread3.join()
                    countermulti += 1
                # MAYBE PUT -> countermulti += 1 <- HERE RATHER THAN ABOVE???????????????????????????????????????
...
if __name__ == "__main__":
    # pwm wird deaktiviert für den gebrauch auf pycharm
    # pwmdefault()
    preparemotor()
